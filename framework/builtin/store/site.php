...
class StoreModuleSite extends WeModuleSite {
	...
	public function __construct() {
		...
		if ((!$_W['isfounder'] || user_is_vice_founder()) && $_W['setting']['store']['status'] == 1 && $_W['store']['from'] != 'payment') {
	...
	public function doWebGoodsBuyer() {
		...
		if ($operate == 'submit_order') {
			...
			if (in_array($goods_info['type'], array(STORE_TYPE_PACKAGE, STORE_TYPE_MODULE, STORE_TYPE_WXAPP_MODULE, STORE_TYPE_API))) {
				if (empty($uniacid)) {
					iajax(-1, '请选择公众号！');
				}
