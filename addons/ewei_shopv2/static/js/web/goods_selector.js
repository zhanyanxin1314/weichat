define(["biz"], function(biz) {
	...
	model.listen = function() {
		...
		model.select2result.click(function() {
			var e = window.event || event;
			if (e.stopPropagation) {
				e.stopPropagation()
			} else {
			...
			model.select2drop.hide();
			model.goodsgroup = $(this).find("div").attr("data-id");
	...
	model.put = function(id) {
		...
		var url = "./index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=util.goods_selector.op&id=" + id + "&nooption=" + model.option_switch + "&column=" + encodeURI($("#goods-selected-list-" + model.name).attr("data-column"));
		if (model.option_switch == 0) {
			url += "&nooption=1"
		}
		var set_color = "btn-danger";
		var htm = '<tr id="goods-selected-goods' + id + '">                <td><img src="' + model.selectedPool[id].thumb + '" style="width: 40px;height: 40px;border: solid #ccc 1px"></td>                <td><p class="title">' + model.selectedPool[id].title + '</p>                <p class="text text-danger">¥' + model.selectedPool[id].marketprice + "</p></td>                <td>" + model.selectedPool[id].total + '</td>                <td><a data-toggle="ajaxModal" href="' + url + '" class="btn ' + set_color + ' btn-sm goods-selector-op" data-id="' + model.selectedPool[id].id + '">商品设置</a>                <a class="btn btn-default btn-sm goods-selector-cancel" data-id="' + id + '">取消</a></td>                </tr>';
	...
	model.getpage = function(page, keywords, goodsgroup) {
		...
		$.ajax({
			url: "./index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=util.goods_selector",
			type: "post",
			data: {
				data: {},
				page: page,
