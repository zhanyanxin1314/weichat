define(['core', 'tpl'], function(core, tpl) {
	...
	modal.getList = function() {
		core.json('commission/log/get_list', {
			page: modal.page,
			status: modal.status
		}, function(ret) {
	...
	modal.getDetaiList = function() {
		core.json('commission/log/orders', {
			'id': modal.applyid,
			page: modal.orderPage
		}, function(ret) {
			var result = ret.result;
			$('#commissioncount').html(result.commissioncount);
			if (result.total <= 0) {
			...
			modal.orderPage++;
			core.tpl('#container', 'tpl_commission_log_order', result, modal.orderPage > 1)
		})
	};
	return modal
});
